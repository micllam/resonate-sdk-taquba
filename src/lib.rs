//! Embedded network for the Resonate Rust SDK: durable execution on an
//! object store. The protocol runs in the SDK's process over a
//! [taquba](https://github.com/micllam/taquba) queue, and its state
//! lives in a [SlateDB](https://slatedb.io) store in the application's
//! own bucket. No Resonate server required. One process owns the store,
//! and a crash resumes from the last committed request at the next open.
//!
//! ```no_run
//! use std::sync::Arc;
...
//! routing group are set on the builder: [`TaqubaNetworkBuilder::pid`]
//! defaults to a generated id and [`TaqubaNetworkBuilder::group`] to
//! `"default"`. [`TaqubaStore::open_with_options`] accepts the queue's
//! [`OpenOptions`](taquba::OpenOptions) (clock, flush interval, queue
//! configuration) and the [`StoreOptions`] (retry timeout, preload
//! limit, poll interval). [`StoreOptions::queue_options`] returns the
//! queue options the store requires, as the base of a caller's own.
//!
//! The [`taquba`] crate is re-exported. The `aws`, `gcp` and `azure`
//! features enable taquba's cloud backends.
//!
//! # Semantics
//!
//! ## A task is a job
...
//! - Every request that writes awaits the store's write-ahead log
//!   flush, so `OpenOptions::flush_interval` is the lower bound on its
//!   latency. SlateDB's default is 100 ms.
...
/// The queue this crate runs on.
pub use taquba;

pub use error::{Error, Result};
pub use network::{TaqubaNetwork, TaqubaNetworkBuilder};
pub use store::{StoreOptions, TaqubaStore};
