//! The SDK's end-to-end suite, run against a `TaqubaNetwork` over an
//! in-memory store. The test bodies are those of the SDK's own
//! `resonate/tests/e2e.rs`; only the construction of the `Resonate`
//! instance differs.
...
/// Open a store for one test.
async fn open_store() -> TaqubaStore {
    TaqubaStore::open(Arc::new(InMemory::new()), "resonate")
        .await
        .expect("store opens")
}
...
/// Build a `Resonate` instance on the store. A unique group per test
/// isolates each test's anycast queue.
fn make_resonate(store: &TaqubaStore) -> Resonate {
    let network = store
        .network()
...
async fn promises_resolve_roundtrip() {
    let r = make_resonate(&open_store().await);
